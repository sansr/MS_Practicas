    ...
    def working(self, system):
        ...
        while True:
            ...
            with repairman.request() as req:
                yield req
                fixing = env.process(fix(env, self.m_name))
                yield fixing
                self.broken = False
            
# Proceso que representa el trabajo de reparar de cada operario
def fix(env, m_name):
    paramExponential = (EXPERIENCE_PER_HOUR*env.now)+MIN_EXPERIENCE
    yield env.timeout(random.expovariate(paramExponential))
    print('%s repaired at %.2f.' % (m_name, env.now))
...
for i in range(total_machines):
    Machine(env, 'Machine %d' % i, system)
